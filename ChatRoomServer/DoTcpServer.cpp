...
DoTcpServer::DoTcpServer(QObject *parent, quint16 port)
	: QTcpServer(parent)
{
	//在指定端口监听任意的地址
...
void DoTcpServer::incomingConnection(long long socketDescriptor)
{
	...
	//用户登录
    connect(client, SIGNAL(userLogin_Signals(QString, QString, int, int, QString, QString)),
            this, SLOT(userLogin_Slots(QString, QString, int, int, QString, QString)));
	//用户退出处理-----修改数据库数据
    connect(client, SIGNAL(userExitHandle_Signals(int)),
            this, SLOT(userExitHandle_Slots(int)));
	//用户断开连接
    connect(client, SIGNAL(disConnected_Signals(int)),
            this, SLOT(disConnected_Slots(int)));
	//聊天消息
    connect(client, SIGNAL(chatMessage_Signals(int, QString)),
            this, SLOT(chatMessage_Slots(int, QString)));
	//用户注册
    connect(client, SIGNAL(userRegister_Signals(int, QString, QString)),
            this, SLOT(userRegister_Slots(int, QString, QString)));
...
//向所有客户端更新所有在线用户的信息，
//只查询数据库响应给客户端即可，此函数不是客户端请求的，所有不用套接字描述符！！！
...
void DoTcpServer::disConnected_Slots(int socketDescriptor)
{
	for (int i = 0; i < clientList.count(); i++)
	{
		...
		if (clientListtRemove->socketDescriptor() == socketDescriptor)
		{
            ...
            qDebug() <<
                        QString::fromLocal8Bit("从列表删除客户端后的总数为：")
                     << clientList.count();
			return;
		}
	}
