	...
	: QTcpSocket(parent)
{
    ...
    connect(this, SIGNAL(readyRead()),
            this, SLOT(dataReceived_Slots()));

    connect(this, SIGNAL(disconnected()),
            this, SLOT(disConnected_Slots()));
}
...
void DoTcpSocket::dataReceived_Slots()
{
	while (bytesAvailable() > 0)
	{
		...
		{
		case USERLOGIN:
            qDebug() << "Client Msg Type:" << "USERLOGIN";
            qDebug() << clientComputerName << clientComputerIP << clientlAccount << clientPassword << clientLoginStatus;
			...
			break;
		case CHATMESSAGE:
            qDebug() << "Client Msg Type:" << "CHATMESSAGE";
			in >> userChatMessage;
			emit chatMessage_Signals(this->socketDescriptor(), userChatMessage);
			break;
		case USERREGISTER:
            qDebug() << "Client Msg Type:" << "USERREGISTER";
			...
			break;
		case USEREXITHANDLE:
            qDebug() << "Client Msg Type:" << "USEREXITHANDLE";
